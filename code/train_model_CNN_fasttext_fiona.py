...
def split_train_test(dataset_path, word2id, split_rate = 0.1):
    ...
    df_dataset = pd.read_table(dataset_path, encoding = 'latin-1', sep = "\t")
...
# # FastText Model
def CNN_FastText_model(max_features_ngram=483141, maxlen=1953, embedding_length=300, nb_classes=2):
    """ CNNFastText """
...
nb_epoch = 3  # number of epoch
embedding_length = 300  # length of word embedding 
gpu_id = 0  # GPU id for GPU model
words_dict_length = 25195  # number of words in dictionary
max_feature = 25195  # number of words in dictionary
max_features_ngram = 483141
...
# model training
df_dataset = pd.read_csv(dataset_path,encoding='latin-1',sep="\t",low_memory=False)
